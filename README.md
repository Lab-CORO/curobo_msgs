...
This single action also drives closed-loop **MPC reactive control**: switch the
planner to `mpc` (`SetPlanner`), set the goal with `TrajectoryGeneration`, then run
`SendTrajectory` to execute the control loop. Use the `mpc_goal` topic
(`geometry_msgs/Pose`) to retarget live during execution.

---

### 2. DataGeneration.action
...
# 2. Set the goal and run the reactive loop (continuously replans, avoids obstacles)
ros2 service call /unified_planner/generate_trajectory curobo_msgs/srv/TrajectoryGeneration \
  "{target_pose: {position: {x: 0.5, y: 0.3, z: 0.4}, orientation: {w: 1.0}}}"
...
Action client example (also used to drive MPC reactive control after switching
the planner to `mpc` and setting a goal):
```python
from rclpy.action import ActionClient
from curobo_msgs.action import SendTrajectory

class ExecClient(Node):
    def __init__(self):
        super().__init__('exec_client')
...
**Trajectory Execution & Reactive Control:**
- `SendTrajectory.action` - Execute a pre-planned trajectory, or drive the
  closed-loop MPC control loop (planner switched to `mpc`)
...
- **NEW**: Added closed-loop MPC reactive control (via `SendTrajectory` + `mpc_goal`)
